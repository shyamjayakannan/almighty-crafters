...
import styles from "./SpecificFileUploader.module.css";
import Delay from "../Delay";

function SpecificFileUploader(props) {
    ...
    function dropFiles(e) {
        ...
        for (const file of e.dataTransfer.files) {
            if (file.type.startsWith(`${props.fileType}/`)) {
    ...
    return (
        <div className={styles.backdrop}>
            <div className={`${styles['drop-zone']} ${drag ? styles['drop-zone__over'] : ''}`} onDrop={dropFiles} onDragOver={onDrag} onDragLeave={onLeave}>
                ...
                <input ref={fileInput} id="file" type="file" className={styles['drop-zone__input']} accept={`${props.fileType}/*`} multiple onChange={addFiles} />
                <span className={styles['drop-zone__prompt']}>Or Drag and Drop files here</span>
                <Delay unmount={!wrongFileType} mount={wrongFileType} delay={250}>
                    <div className={styles.backdrop2}>
                        <div className={`${styles["wrong-file-type"]} ${!wrongFileType ? styles["wrong-file-type__exit"] : ''}`}>
                            Please Upload only {props.fileType} files
                            <button onClick={() => setWrongFileType(false)}>OK</button>
                        </div>
                    </div>
                </Delay>
            </div>
...
export default SpecificFileUploader;